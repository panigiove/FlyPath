...
use wg_2024::packet::NackType::{ErrorInRouting, Dropped};
...
mod tests {
    ...
    fn test_default_command_add_sender(){
        ...
        d2_command_send.send(DroneCommand::AddSender(1, d1_send.clone())).unwrap();
        d1_command_send.send(DroneCommand::AddSender(2, d2_send.clone())).unwrap();
    
        let packet_from_c1_to_d2 = Packet::new_fragment(SourceRoutingHeader::with_first_hop(vec![10,1,2]), 1, Fragment::from_string(1, 1, "heyy".to_string()));
        ...
        assert!(packet.is_ok());
        if let Ok(packet) = packet {
            assert_eq!(packet.pack_type, PacketType::Nack(wg_2024::packet::Nack { fragment_index: 1, nack_type: wg_2024::packet::NackType::DestinationIsDrone }));
        }
    ...
    fn test_default_command_set_dpr(){
        ...
        // send packet
        let packet_from_c1_to_d2 = Packet::new_fragment(SourceRoutingHeader::with_first_hop(vec![10,1,2]), 1, Fragment::from_string(1, 1, "heyy".to_string()));
        d1_command_send.send(DroneCommand::SetPacketDropRate(1.0)).unwrap();
        d1_send.send(packet_from_c1_to_d2.clone()).unwrap();
        ...
        assert!(packet.is_ok());
        if let Ok(packet) = packet {
            assert_eq!(packet.pack_type, PacketType::Nack(wg_2024::packet::Nack { fragment_index: 1, nack_type: wg_2024::packet::NackType::Dropped }));
        }
        ...
        if let Ok(event) = event {
            assert_eq!(event, DroneEvent::PacketDropped(packet_from_c1_to_d2.clone()));
        }
        assert!(from_d1_event_recv.is_empty());
    }
    ...
    fn test_default_command_remove_crash(){
        ...
        assert!(packet.is_ok());
        if let Ok(packet) = packet {
            assert_eq!(packet.pack_type, PacketType::Nack(wg_2024::packet::Nack { fragment_index: 1, nack_type: wg_2024::packet::NackType::ErrorInRouting((2)) }));
        }

        // make d2 crash and send a packet
        let ack_from_d2_to_c1 = Packet::new_ack(SourceRoutingHeader::with_first_hop(vec![22,2,1,10]), 2, 2);
